 ...
 *
 * É o gêmeo do briefing.gs (identidade visual), com as perguntas do site. Cada
 * formulário tem a sua própria planilha e a sua própria implantação: assim um
 * ajuste em um não republica o outro.
 *
...
const RESUMO_HEADERS = ['Data', 'Empresa / marca', 'Nome', 'E-mail', 'WhatsApp', 'Tipo de projeto', 'Objetivo', 'Prazo', 'Briefing'];
...
const SECTIONS = [
  ...
  },
  {
    ...
    fields: [
      ['q_objetivo', 'O que o site mais precisa fazer?'],
      ['q_tipo_projeto', 'Tipo de projeto (derivado da resposta acima)'],
      ['q_acao', 'O que a pessoa deve fazer ao entrar no site?'],
      ['q_tem_site', 'Já têm site hoje?'],
      ['q_site_atual', 'O que funciona e o que incomoda no site atual']
    ]
  },
  {
    title: 'Sobre a empresa',
    fields: [
      ['q_o_que_faz', 'O que a empresa faz, em poucas palavras?'],
      ['q_tempo_mercado', 'Há quanto tempo existem'],
      ['q_melhor_escolha', 'Por que são a melhor escolha pro cliente'],
      ['q_numeros', 'Números que impressionam']
    ]
  },
  {
    title: 'Quem vai visitar o site',
    fields: [
      ['q_visitante', 'Quem é a pessoa que vai entrar no site?'],
      ['q_receio', 'Maior dúvida ou receio antes de fechar'],
      ['q_convence', 'O que convence o cliente a confiar']
    ]
  },
  {
    title: 'Conteúdo que vocês já têm',
    fields: [
      ['q_textos', 'Textos prontos ou a escrever'],
      ['q_fotos', 'Fotos próprias'],
      ['q_depoimentos', 'Depoimentos de clientes'],
      ['q_logos_clientes', 'Logos de clientes ou parceiros']
    ]
  },
  {
    title: 'O que o site precisa ter',
    fields: [
      ['q_recursos', 'Itens marcados'],
      ['q_recursos_outro', 'Outro (o que mais o site precisa ter)']
    ]
  },
  {
    title: 'Visual e referências',
    fields: [
      ['q_referencias', 'Sites de referência'],
      ['q_cores_marca', 'Cores da marca'],
      ['q_clima', 'Clima que o site deve passar'],
      ['q_evitar', 'Cor ou estilo a evitar']
    ]
  },
  {
    title: 'Contato e detalhes práticos',
    fields: [
      ['q_canais', 'Por onde o cliente vai falar com vocês'],
      ['q_redes', 'Redes sociais para linkar'],
      ['q_dominio', 'Domínio já registrado?']
    ]
  },
  {
    title: 'Pra fechar o combinado',
    fields: [
      ['q_prazo', 'Pra quando precisa do site no ar'],
      ['q_aprovador', 'Quem aprova o projeto'],
...
/**
 * Perguntas que só aparecem em parte dos casos. Quando vêm em branco, é porque
 * a condição não foi atendida — a linha some da aba em vez de virar ruído.
 */
const CONDICIONAIS = ['q_site_atual', 'q_tipo_projeto'];

/** Teste rápido de que a implantação está no ar: abra a URL no navegador. */
function doGet() {
  return json({ ok: true, service: 'briefing-site' });
}
...
function createClientSheet(ss, fields) {
  ...
  rows.push(['Briefing de site', String(fields.empresa || '').trim()]);
  ...
  SECTIONS.forEach(section => {
    ...
    section.fields.forEach(pair => {
      ...
      // campos condicionais em branco ("Outro", site atual) só poluiriam a leitura
      if (value === '' && isConditional(pair[0])) return;
...
function isConditional(key) {
  return key.indexOf('_outro') > -1 || CONDICIONAIS.indexOf(key) > -1;
}
...
function notify(fields, url) {
  ...
  try {
    MailApp.sendEmail({
      to: to,
      subject: 'Novo briefing de site: ' + fields.empresa,
      htmlBody:
        '<p><strong>' + fields.nome + '</strong> (' + fields.empresa + ') enviou o briefing do site.</p>' +
        '<p>Tipo: ' + (fields.q_tipo_projeto || '—') + '<br>Objetivo: ' + (fields.q_objetivo || '—') + '</p>' +
